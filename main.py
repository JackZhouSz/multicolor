...
def main():
    ...
    parser.add_argument("--depth", type=str, help="Path to the depth map file.")
    args = parser.parse_args()
    if args.settings is not None:
        settings_filename = str(args.settings.name)
    process(
        settings_fn=settings_filename, input_image=args.input, depth_image=args.depth
    )
...
def compansate_illumination(settings, target_image, device):
    ...
    target_image_com = target_image_com / torch.amax(target_image_com, dim=(1, 2)).view(
        3, 1, 1
    )
...
def process(settings_fn, input_image=None, depth_image=None):
    ...
    if target_image.shape[1] < resolution[0] or target_image.shape[2] < resolution[1]:
        target_image = odak.learn.tools.zero_pad(
            target_image, [resolution[0], resolution[1]]
        )
    else:
    ...
    if target_depth.shape[0] < resolution[0] or target_depth.shape[1] < resolution[1]:
        target_depth = odak.learn.tools.zero_pad(
            target_depth, [resolution[0], resolution[1]]
        )
    else:
...
def save(
    ...
    for depth_id in range(targets.shape[0]):
        odak.learn.tools.save_image(
            "{}/target_{:02d}.png".format(directory, depth_id),
            targets[depth_id] * intensity_scale,
            cmin=0.0,
    ...
    for frame_id in range(hologram_phases.shape[0]):
        ...
        for depth_id in range(targets.shape[0]):
            odak.learn.tools.save_image(
                "{}/reconstruction_frame_{:02d}_depth_{:03d}.png".format(
                    directory, frame_id, depth_id
                ),
        ...
        complex_field = odak.learn.wave.generate_complex_field(
            torch.ones_like(phase_normalized), phase_normalized
        )
    ...
    odak.learn.tools.save_torch_tensor(
        "{}/laser_powers.pt".format(directory), laser_powers
    )
    odak.learn.tools.save_torch_tensor(
        "{}/channel_powers.pt".format(directory), channel_powers
    )
    data = {
        "targets": targets,
        "target": target_image,
        ...
        "hologram amplitudes": hologram_amplitudes,
        "hologram phases": hologram_phases,
        "settings": settings,
    }
    odak.learn.tools.save_torch_tensor("{}/data.pt".format(directory), data)
    print(
        "Output stored at {}. Check `odak.log` for more information.".format(directory)
    )
